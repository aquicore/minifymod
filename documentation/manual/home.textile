...
h2. Response optimization for Play
...
The latest version of this module can be fetched from github : "https://github.com/maklemenz/play-minifymod":https://github.com/maklemenz/play-minifymod

h2. How To Use It

Let's say you have a simple controller wich delivers a hugh site:

bc. public class MustBeOptimized extends Controller {
	public static void renderMyHughTemplate() {
		render("largeSite.html");
	}
}

h3. Minify the response
...
bc. minifymod.moduleEnabled = true
...
By adding the line @With(MinifyAndGzipResponse.class) your controller gets another method called compress(). This method is triggered after your template is rendered because it is annotated with @Finally.

This method gets a the outputstream from the current response (response.out) and makes a String with the content of this stream.

After that the content will be minified with googles htmlcompressor or the yui compressor.

The outputstream is replaced by a gzipped one and the necessary headers will be set.

At the end the it writes back the minified content to response.out and after that, everything goes its normal way.
